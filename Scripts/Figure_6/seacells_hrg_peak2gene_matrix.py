...
rna_meta_ad_hrg = sc.read(os.path.join(work_dir, "Matrix_Multiome_HRG_rna_meta_ad.h5ad"))
print(rna_meta_ad_hrg)

#-----Running Peak to Gene Linkage-----#
print("Running Peak to Gene Linkage")
gene_set = rna_meta_ad_hrg.var_names
gene_peak_cors = SEACells.genescores.get_gene_peak_correlations(atac_meta_ad, rna_meta_ad_hrg, 
                                           path_to_gtf='/projects/b1217/HHA/Dictys/Inputs/gene.gtf', 
                                           span=500000, 
                                           n_jobs=62,
                                           gene_set=gene_set)
print("Saving Results")
pickle.dump(gene_peak_cors, open(os.path.join(work_dir, 'hrg_gene_peak_cors.pkl'), 'wb'))
print("Done")
